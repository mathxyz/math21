...
namespace math21 {
    namespace ad {
        ...
        void op_num_add::df(const Set &X, NumN x, NumN y, Set &output, VariableMap &data) const {
            if (X.contains(x)) {
        ...
        void op_num_add::cr(const Set &X, NumN x, NumN y, NumN dy, Set &output, VariableMap &data) const {
            if (X.contains(x)) {
                op_num_constant numConstant;
                numConstant.f(X, output, data);
                data.at(output(1)).getValue() = 1;
        ...
        void op_num_add::backward(const Set &X, NumN x, NumN y, NumN dy, Set &output, VariableMap &data) const {
            if (X.contains(x)) {
                op_num_constant numConstant;
        ...
        void op_num_add::f(const Set &X, Set &output, VariableMap &data) {
            NumN y = data.createV("op_num_add(x)");
            data.at(y).setf(this);
            data.at(y).setX(X);
        ...
        void op_num_add::fv(const Set &X, const Set &Y, VariableMap &data) const {
            if (X.isEmpty()) {
                return;
            }
            NumN y = Y(1);
        ...
        void op_num_add::setSize(const Set &X, const Set &Y, VariableMap &data) const {
        }
